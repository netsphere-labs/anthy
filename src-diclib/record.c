 ...
 * 文字列(xstr)をキーにして高速に行(column)を検索することができる．
...
struct record_stat {
  ...
  int column_dirty; /* cur_column が保存の必要があるか */
  /**/
  int is_anon;
  ...
  int lock_fd;      /* 差分ファイルロック用のファイルディスクリプタ */
 ...
 *         column の key を intern された xstr として使う。
 *         column に value は持たない。
 ...
 *     オープンしていてはいけない。
 *  追加と削除:
 *     追加はすでにメモリ上で更新された column をコミットによって
 *     メモリに書き出すため、
 *       1. コミット対象 column 以外を差分ファイルの情報で
 *       2. コミット対象 column を差分ファイルに書き出し
 *     とする。削除はまだメモリ上に column が残っている状態でコミット
...
/* 
 * トライの実装
 * struct trie_nodeのうちcolumn以外の部分とcolumn.keyを使用
 * 削除の時はtrie_column_freeを使ってcolumnの内容を解放
 */
...
/* 
 * ノードを見つけると削除する
 * 内部でtrie_column_freeを呼び、キーを含むデータ部分をfreeする
 *
 * データとノードを削除する。
 ...
 *   フラッシュがあると、 cur_column と全ての xstr は無効になる。
...
anthy_select_next_column(void)
{
  ...
  /* sync_add() で cur_column が無効になることがあるので、
   * たとえ column_dirty でも sync_add() しない
   */
  rst->column_dirty = 0;
